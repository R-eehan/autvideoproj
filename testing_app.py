...
def login():
	...
	if request.method == 'POST':
		...
		with requests.get('https://api.browserstack.com/automate/plan.json', auth=(username, password), timeout= 5) as check:
...
def automate():
	...
	for session_id in session_ids:
		session_id_response_data = requests.get(f"{automate_url}{session_id}.json", auth=(username, password), timeout=10).json()
